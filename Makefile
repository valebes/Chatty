...
FILE_DA_CONSEGNARE=Makefile chatty.c client.c message.h ops.h stats.h config.h \
		   DATA/chatty.conf1 DATA/chatty.conf2 connections.h Doc/* logo.png \
			 Relazione_Valerio_Besozzi.pdf utlist.h utility.h utility.c uthash.h \
			 tpool.h tpool.c  user.h user.c task.h task.c parser.h parser.c listener.c \
			 connections.c script_bash.sh

# inserire il nome del tarball: es. NinoBixio
TARNAME=ValerioBesozzi
...
chatty: chatty.o libchatty.a $(INCLUDE_FILES)
	$(CC) $(CFLAGS) $(INCLUDES) $(OPTFLAGS) $(LDFLAGS) -o $@ $^ $(LIBS) #-fsanitize=address
